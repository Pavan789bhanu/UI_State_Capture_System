...
class PlannerAgent:
    ...
    async def plan_task(
        ...
        Returns:
            ...
            - "app_url": str
        """
        # Get relevant few-shot examples based on the task
        few_shot_examples = self.few_shot_generator.get_examples_for_task(task, num_examples=3)
        ...
        system_prompt = (
            ...
            "Study the examples below to understand workflow patterns, then create a similar plan:\n\n"
            f"{few_shot_prompt}\n\n"
            "Step types:\n"
        ...
        user_prompt = (
            ...
            "- CRITICAL: Include verification step at the end to confirm task completion\n"
            "- If task requires prerequisites (e.g., workspace before project), include those steps first\n"
            "- For document creation: create → name → add content → verify document exists\n"
            "- For project/issue creation: navigate to section → click new → fill form → submit → verify created\n"
            "- ALWAYS end with a clear verification step that checks the final result\n\n"
