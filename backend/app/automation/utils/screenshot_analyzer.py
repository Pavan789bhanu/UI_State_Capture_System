...
class ScreenshotAnalyzer:
    ...
    def deduplicate_screenshots(self, screenshot_paths: List[str], threshold: int = None, remove_duplicates: bool = None) -> List[str]:
        ...
        Args:
            ...
            threshold: Hash difference threshold (default from config: {settings.SCREENSHOT_DEDUPLICATION_THRESHOLD} bits)
            ...
            remove_duplicates: If True, delete duplicate files from disk (default from config: {settings.SCREENSHOT_DELETE_DUPLICATES})
        ...
        if threshold is None:
            threshold = settings.SCREENSHOT_DEDUPLICATION_THRESHOLD
        if remove_duplicates is None:
            remove_duplicates = settings.SCREENSHOT_DELETE_DUPLICATES
...
Be specific and reference visible UI elements (buttons, forms, text, etc.)."""
    ...
    async def generate_narrative(
        ...
        if settings.SCREENSHOT_DEDUPLICATION_ENABLED:
            log(f"Deduplication enabled: threshold={settings.SCREENSHOT_DEDUPLICATION_THRESHOLD} bits, delete={settings.SCREENSHOT_DELETE_DUPLICATES}")
